...
class Action extends mainAction
{
	...
	public function publicstoreAjax()
	{
		...
		if($modenum != ''){
			...
			$nas		= $this->flow->billwhere($this->adminid, $atype); // 2018年3月26日14:42:13 squid 要解析本函数的作用
			$_wehs		= $nas['where'];
	...
	*/
	public function publicsaveAjax()
	{
		...
		$submditfi 			= $this->rock->jm->base64decode($this->post('submitfields_postabc')); // squid 要操作的字段
