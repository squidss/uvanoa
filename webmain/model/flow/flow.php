...
class flowModel extends Model
{
	...
	/**
	 * @squid 2018年4月2日17:42:48  初始化本flowmodel的数据，保存一些变量到自身或者其他操作，方便整个继承使用
	 * @param unknown $num 模块名
	 * @param unknown $id 模块记录的id(主键)
	 */
	...
	*/
	public function loaddata($id, $ispd=true)
	{
		...
		$this->rs 		= $this->getone($id); // @squid 2018年4月4日09:40:56  获得当前模型的id为$id的字段记录
		$this->uname	= '';
		...
		$this->urs 		= $this->adminmodel->getone($this->uid,'id,name,deptid,deptname,deptallname,face,ranking,superid,superpath,superman,deptpath'); // @squid 2018年4月4日09:41:26 获得用户信息
		...
		$this->sericnum	= '';
		$this->billrs 	= $this->billmodel->getone($this->mwhere); // @squid 2018年4月4日09:41:52  获得当前表单flow_bill的信息
		if($this->billrs){
	...
	*/
	public function billwhere($uid, $lx)
	{
		...
		// squid 上面的fwhere是【  and {asqom}`uid`='1'】,当查询客户申请使用的时候
	...
	*/
	public function getflowwhere($uid, $num)
	{
		...
		if(!$rs)return $where;
		// @squid 2018年3月26日14:09:37 加了一个注释
		$where = $this->wheremodel->getwherestr($rs, $uid, $this->flowviewufieds); // rs是表flow_where的一条记录， $this->flowviewufieds是字符串'uid'
		return $where;
	}
