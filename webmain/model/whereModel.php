...
class whereClassModel extends Model
{
	...
	*/
	...
	/**
	 * @author squid 2018年3月26日14:35:42  备注方法返回
	 * 这个返回表flow_where的三个字段解密的结果，分别是`wheresstr`对应表条件、`whereustr`用户条件、`wheredstr`对应部门条件
	 * @param unknown $id
	 * @param number $uid
	 * @param string $fid
	...
	public function getflowwhere($id, $uid=0, $fid='')
	{
		...
		if($fid=='')$fid='`uid`';
		$modeid 		= (int)$rs['setid'];
		$this->moders 	= m('flow_set')->getone($modeid); // squid 获得模块的字段信息
	...
	/**
	 * @squid 2018年3月26日14:39:46
	 * 本方法返回一个根据表flow_where的三个字段的条件，当$lx没有传值而且flow_where的wheresstr字段值不为空的时候返回一个where条件
	 * @param unknown $id
	 * @param number $uid
	 * @param string $fid
	 * @param number $lx
