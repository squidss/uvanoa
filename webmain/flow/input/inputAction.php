...
class inputAction extends Action
{
	...
	*/
	public function saveAjax()
	{
		...
		if(!$this->isempt($ss))$this->backmsg($ss); // @squid 2018年4月2日16:46:59 开发者自定义的before是否有信息返回，如果有的话，终止保存并返回开发者定义的信息
		...
		$this->flow->loaddata($id, false); // @squid 2018年4月2日19:19:49  生成flow的其他表的数据
		$this->flow->submit($subna); // 判断当前表单的流程
		
		$this->backmsg('', $msg, $id);
	}
	
	/*
	 * @squid 2018年4月2日16:44:19 获得更新的数据，bos是旧的数据，用新的数据数组覆盖旧的数组数组，如果新的有部分字段没有变动，直接使用旧的字段数据
	 */
	...
	{
		$this->ismobile = 1;
		// @squid 2018年3月28日16:38:34 将判断动作和条件存到模板数据上，如果动作和条件不存在，则为空，在模板js上面进行处理
		$this->smartydata['sq_option']['menunum'] = $this->request('menunum', '');
		$this->smartydata['sq_option']['param'] = $this->request('param', '');
	...
	/**
	 * @squid 2018年3月26日17:32:18 本方法用于组装表单，会根据表单的条件之类生成表单页面
	 */
	//$lutype=1高级搜索用的
	private function luactions($slx=0, $stwhe='', $lutype=0)
	{
		...
		$zbshu			= 0;
		$tableas		= false;
		if(!isempt($moders['tables'])){ // 是否有子表
			$tableas = explode(',', $moders['tables']);
			$zbshu	 = count($tableas); // @squid 子表数
		}
		...
		if($this->ismobile==0){
			...
			if($tableas && $slx==0){
				foreach($tableas as $k1=>$tableass){
					$zbstr 	 = m('input')->getsubtable($modeid,$k1+1,1,1); // @squid 获得子表的html
					if($zbstr!=''){
